...
function CartPage() {
  ...
  const handleApplyCode = async (codeToVerify?: string) => {
    ...
    try {
      let res: { valid: boolean; discountPercent: number; label?: string };
      try {
        res = await runVerifyDiscount({ data: { code } });
      } catch {
        const localCode = sessionStorage.getItem("active_discount_code");
        const localPercent = Number(sessionStorage.getItem("active_discount_percent") || 0);
        res = {
          valid: localCode?.toUpperCase() === code.toUpperCase() && localPercent > 0,
          discountPercent: localPercent,
          label: "عرض Bali+",
        };
      }
