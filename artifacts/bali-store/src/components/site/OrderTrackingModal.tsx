...
export function OrderTrackingModal({ open, onOpenChange, defaultQuery = "" }: Props) {
  ...
  const handleSearch = async (e?: React.FormEvent) => {
    ...
    setSearching(true);
    try {
      let res: TrackedOrder[];
      try {
        res = await runTrack({ data: { query: query.trim() } });
      } catch {
        const needle = query.trim();
        const { data, error } = await supabase
          .from("orders")
          .select("id,order_number,customer_name,phone,address,status,total,created_at,order_items(id,product_id,product_name,quantity,unit_price)")
        ...
        res = ((data ?? []).filter((order) =>
          numeric && needle.length < 7
            ? String(order.order_number) === needle
            : String(order.phone).includes(digits.slice(-8)),
